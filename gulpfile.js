...
gulp.task('inject', ['wiredep', 'styles','clean-code'], function() {
...
gulp.task('build', ['optimize', 'images', 'fonts'], function() {
  ...
  return gulp
    .src([config.build + '**', '!' + config.build + 'app/'])
    .pipe(gulp.dest('./'));  
});
...
gulp.task('optimize', ['inject'], function() {
  ...
  return gulp
    ...
    // Get the css
    .pipe(cssFilter)
    .pipe($.if(!args.n, $.minifyCss()))
    .pipe(cssFilter.restore())
    // Get the custom javascript
    .pipe(jsAppFilter)
    .pipe($.ngAnnotate({ add: true }))
    .pipe($.if(!args.n, $.uglify()))
    .pipe(getHeader())
    .pipe(jsAppFilter.restore())
    // Get the vendor javascript
    .pipe(jslibFilter)
    .pipe($.if(!args.n, $.uglify())) // another option is to override wiredep to use min files
